...
public class FisheyeTreeFilter extends GroupAction {
     ...
     * Action is run.
     */
    public FisheyeTreeFilter(Visualization vis, String group) {
        this(vis, group, 1);
    }
     ...
     */
    public FisheyeTreeFilter(Visualization vis, String group, int distance) {
        this(vis, group, Visualization.FOCUS_ITEMS, distance);
    }
     ...
     * nodes past which nodes will not be visible nor expanded.
     */
    public FisheyeTreeFilter(Visualization vis, String group, String sources, int distance)
    {
        super(vis, group);
        m_sources = sources;
    ...
    public void run(double frac) {
        Tree tree = ((Graph)getVisualization().getGroup(m_group)).getSpanningTree();
        m_divisor = tree.getNodeCount();
        m_root = (NodeItem)tree.getRoot();
        
        // mark the items
        Iterator<?> items = getVisualization().visibleItems(m_group);
        while ( items.hasNext() ) {
        ...
        Iterator<?> iter = getVisualization().items(m_sources, m_groupP);
        while ( iter.hasNext() )
            visitFocus((NodeItem)iter.next(), null);
        visitFocus(m_root, null);

        // mark unreached items
        items = getVisualization().visibleItems(m_group);
        while ( items.hasNext() ) {
            VisualItem item = (VisualItem)items.next();
