...
public class AxisLabelLayout extends Layout {
     ...
     */
    public AxisLabelLayout(Visualization vis, String group, int axis, ValuedRangeModel values)
    {
        this(vis, group, axis, values, null);
    }
     ...
     */
    public AxisLabelLayout(Visualization vis, String group, AxisLayout layout) {
        this(vis, group, layout, null, 50);
    }
     ...
     * @param bounds the layout bounds within which to place the axis marks
     */
    public AxisLabelLayout(Visualization vis, String group, AxisLayout layout, Rectangle2D bounds) {
        this(vis, group, layout, bounds, 50);
    }
    ...
    protected VisualTable getTable() {
        TupleSet ts = getVisualization().getGroup(m_group);
        if ( ts == null ) {
            Schema s = PrefuseLib.getAxisLabelSchema();
            VisualTable vt = getVisualization().addTable(m_group, s);
            vt.index(VALUE);
            return vt;
