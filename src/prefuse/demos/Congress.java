...
public class Congress extends JPanel {
    ...
    public Congress(Table t) {
        ...
        final Visualization vis = new Visualization("Congress");
        m_vis = vis;
        ...
        // set up the actions
        AxisLayout xaxis = new AxisLayout(vis, group, "State Code",
                Constants.X_AXIS, VisiblePredicate.TRUE); 
        AxisLayout yaxis = new AxisLayout(vis, group, RECEIPTS,
        ...
        AxisLabelLayout ylabels = new AxisLabelLayout(vis, "ylab", yaxis, m_ylabB);
        NumberFormat nf = NumberFormat.getCurrencyInstance();
        nf.setMaximumFractionDigits(0);
        ylabels.setNumberFormat(nf);
        
        AxisLabelLayout xlabels = new AxisLabelLayout(vis, "xlab", xaxis, m_xlabB, 15);
        ...
        DataColorAction color = new DataColorAction(vis, group, "Party",
                Constants.ORDINAL, VisualItem.STROKECOLOR, palette);
        
        int[] shapes = new int[]
            { Constants.SHAPE_RECTANGLE, Constants.SHAPE_DIAMOND };
        DataShapeAction shape = new DataShapeAction(vis, group, "Senate", shapes);
        
        Counter cntr = new Counter(vis, group);
        
        ActionList draw = new ActionList(vis);
        draw.add(cntr);
        draw.add(color);
        draw.add(shape);
        draw.add(xaxis);
        draw.add(yaxis);
        draw.add(ylabels);
        draw.add(new ColorAction(vis, group, VisualItem.FILLCOLOR, 0));
        draw.add(new RepaintAction(vis));
        vis.putAction("draw", draw);

        ActionList update = new ActionList(vis);
        update.add(new VisibilityFilter(vis, group, filter));
        update.add(cntr);
        update.add(xaxis);
        update.add(yaxis);
        update.add(ylabels);
        update.add(new RepaintAction(vis));
    ...
    private class Counter extends GroupAction {
        public Counter(Visualization vis, String group) {
        	super(vis);
        	this.setGroup(group);
            //super(group);
        }
