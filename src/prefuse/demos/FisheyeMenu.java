...
public class FisheyeMenu extends Display {
     ...
     */
    public FisheyeMenu() {
        super(new Visualization("FisheyeMenu"));
        getVisualization().addTable(ITEMS, m_items);
        ...
        getVisualization().setRendererFactory(new DefaultRendererFactory(renderer));
        ...
        ColorAction colors = new ColorAction(getVisualization(), ITEMS, VisualItem.TEXTCOLOR);
        ...
        init.add(new VerticalLineLayout(getVisualization(), m_maxHeight));
        init.add(colors);
        init.add(new RepaintAction(getVisualization()));
        getVisualization().putAction("init", init);

        // fisheye distortion based on the current anchor location
        ActionList distort = new ActionList(getVisualization());
        Distortion feye = new FisheyeDistortion(getVisualization(),0,m_scale);
        distort.add(feye);
        distort.add(colors);
        distort.add(new RepaintAction(getVisualization()));
        getVisualization().putAction("distort", distort);
    ...
    public class VerticalLineLayout extends Layout {
        ...
        public VerticalLineLayout(Visualization vis, double maxHeight) {
        	super(vis);
        ...
        public void run(double frac) {
            // first pass
            double w = 0, h = 0;
            Iterator<?> iter = getVisualization().items();
            while ( iter.hasNext() ) {
