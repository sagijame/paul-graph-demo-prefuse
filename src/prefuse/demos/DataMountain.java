...
public class DataMountain extends Display {
    ...
    public DataMountain(Table t) {
        super(new Visualization("DataMountain"));
        getVisualization().addTable("data", t);
        ...
        getVisualization().setRendererFactory(new DefaultRendererFactory(nodeRenderer));
        
        ActionList init = new ActionList(getVisualization());
        init.add(new RandomLayout(getVisualization()));
        init.add(new DataMountainSizeAction(getVisualization()));
        getVisualization().putAction("init", init);
        
        ActionList update = new ActionList(getVisualization());
        update.add(new DataMountainSizeAction(getVisualization()));
        update.add(new ColorAction(getVisualization(), "data", VisualItem.STROKECOLOR) {
            public int getColor(VisualItem item) {
                return ColorLib.rgb((item.isHover() ? 255 : 0), 0, 0);
            }
        });
        update.add(new RepaintAction(getVisualization()));
        getVisualization().putAction("update", update);

        // we run this to make sure the forces are stabilized
        ActionList preforce = new ActionList(getVisualization(), 1000);
        preforce.add(new DataMountainForceLayout(getVisualization(),true));
        getVisualization().putAction("preforce", preforce);

        // this will cause docs to move out of the way when dragging
        final ForceDirectedLayout fl = new DataMountainForceLayout(getVisualization(),false);
        ActivityListener fReset = new ActivityAdapter() {
            public void activityCancelled(Activity a) {
                fl.reset(); 
             } 
        };
        ActionList forces = new ActionList(getVisualization(), Activity.INFINITY);
        forces.add(fl);
        forces.add(update);
        forces.addActivityListener(fReset);
        getVisualization().putAction("forces", forces);
        ...
        nodeRenderer.getImageFactory().preloadImages(getVisualization().items(),"image");
        
        // initialize and present the interface
        getVisualization().run("init");
        getVisualization().runAfter("preforce", "update");
        getVisualization().run("preforce");
    }
    ...
    public class DataMountainSizeAction extends SizeAction {
        public DataMountainSizeAction(Visualization vis) {
			super(vis);
		}
    ...
    public class DataMountainForceLayout extends ForceDirectedLayout {
        ...
        public DataMountainForceLayout(Visualization vis, boolean enforceBounds) {
            super(vis, "data",enforceBounds,false);
        ...
        public void reset() {
            Iterator<?> iter = getVisualization().visibleItems(m_nodeGroup);
            while ( iter.hasNext() ) {
        ...
        protected void initSimulator(ForceSimulator fsim) {
            ...
            TupleSet t = (TupleSet)getVisualization().getGroup(m_group);
